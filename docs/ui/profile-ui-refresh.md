...
- CmdK / Ctrl+K "Ask about Pius" launcher **as a visual shell only**.
...
- ~~Surfacing project gallery imagery on the detail page with graceful missing-media fallbacks.~~
  **Done** - implemented in the detail rebuild (media grid + lightbox + per-image `onError`); see
  Outcomes.
- Evidence result **shell** content and CmdK panel polish.
...
fabricates RAG answers.** The CmdK/Evidence surface is a client shell only.
...
3. `feat(web): refresh profile experience` - hero repaint + CmdK launcher chip, bento capabilities
   grid, experience evidence rows.
4. `feat(web): improve project presentation` - differentiated cards + graceful gallery media.
5. `feat(web): add assistant command shell` - CmdK/Ctrl+K handler + placeholder Evidence panel;
...
- Statement-led hero (gradient-emphasis headline, facts panel), capabilities **bento** (niche tile
  ...
  resume-style experience with metric pills, centered contact card.
- **CmdK assistant shell** (placeholder only - no backend/LLM/keys/RAG): an "Ask about Pius"
