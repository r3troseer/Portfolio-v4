...
## Locked decision
The CmdK "Ask about Pius" launcher is a **single element that flies** between its hero
...
- Flight: time-based, `easeInOutCubic` for progress; `DUR = clamp(430, 560, 560 - scrollVel*26)`.
  `scrollVel` = EMA `0.6*prev + 0.4*(|deltay|/deltat)`, decay `*0.85` when idle > 120ms.
- Position eases: `eOut = 1-(1-p)3` (left, radius, padding), `eInOut` cubic (top).
- Corner inset: **24px** desktop, **16px** mobile. `dockTop = innerHeight - edge - pillHeight`.
- Rest->dock shape (x eOut): radius `14 -> 999px`, h-padding `20 -> 15`, font `1 -> 0.9rem`,
...
- Colour: rest teal sheen `linear-gradient(180deg,#7dffe6,#64ffda 58%,#50e7ce)`, border
  ...
  `ece = 1-(1-ec)2` -> dock `bg rgba(17,22,34,0.60)`, border `rgba(255,255,255,0.14)`, text
