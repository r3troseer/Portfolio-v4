...
2. **The 1,200-char cap is a hard character slice applied before marker validation** (`schemas.py`: `normalize_answer_prose(answer)[:ANSWER_MAX_LENGTH]`, then `parse_prose_markup` on the truncated string). Three outcomes when the model runs long: (a) the cut lands mid-`[[evidence_id]]` while that id is in `citation_ids` → the dangling marker no longer parses, the orphan-citation check raises `AnswerOutputError` → **502 for a well-formed answer**; (b) the cut removes the only marker → the "requires at least one marker" check → same 502; (c) the cut lands in plain text → validation passes and the served prose ends mid-sentence (the client's `stripProseMarkup`/`renderProse` strip dangling `[[`/`==`, but can't un-chop the sentence). Mitigations in its favor: the prompt asks for 2–4 sentences so 1,200 chars is generous headroom, and cases (a)/(b) fail closed rather than serving broken text. **Decision (author): truncate safely rather than reject** — keep an answer on screen instead of trading "too long" for a 502. The working shape has three requirements: (a) cut only at a sentence-end/whitespace position that lies **outside every `[[...]]`/`==...==` span**; (b) order matters — run the marker/citation discipline checks against the model's *full* output first (that is what catches real model errors), *then* truncate safely, *then* **recompute `citation_ids` from the truncated prose** so the served payload is internally consistent by construction (truncating first manufactures fake "orphan citation" 502s); (c) if truncation would drop *every* marker (the only citation sits past the 1,200-char boundary), still fail closed — an "answered" response with zero citations violates the contract, and 1,200 chars of preamble before the first grounded claim is not worth serving. Add tests for: plain-text tail cut, limit landing inside a marker, all-markers-past-limit, and citations recomputed to match served prose.
...
6. ~~**Node engines churn.**~~ **Retracted** (author feedback): the `24.x` pin is intentional — it matches the local dev machine, Node 20 reached end-of-life in April 2026 (so `>=20.19.0` would have permitted EOL runtimes), and CI/Vercel align on 24. Only note kept: npm does not enforce `engines` by default (this review's sandbox ran the build on Node 22 without complaint), so add `engine-strict=true` to `.npmrc` if the pin should be enforced rather than documentary.
