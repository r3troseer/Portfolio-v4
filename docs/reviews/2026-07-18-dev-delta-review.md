...
**Verified at `e930309`:** API suite — **151 tests, OK** (up from 84; running them required a ≥50-char `DJANGO_SECRET_KEY`, i.e. the new startup guard blocked this reviewer first, as designed); `build_evidence_index --check` green (19 records, ESG excluded); web `npm ci` + production build green (29 assets, main chunk 283 kB / 91.6 kB gzip; measured homepage critical path 101.6 kB gzip, down from 121.5); answer-contract fixture suite green (5 valid / 12 invalid). Includes a requested **slop / unnecessaries audit** (Part IV).
...
4. **Deterministic reranking (`reranking.py` + rewritten `retrieval.py`)** — retrieval is now an explicit two-stage pipeline: lexical candidate generation (pool = 3×`top_k`, capped at 20, stopword-filtered) feeding a breadth/phrase-aware integer rerank that is deliberately non-monotone vs the lexical order (capped lexical carry), fully explainable (`components` + `reasons` per row), stable tie-breaks, tested. `POST /api/retrieve/` itself now returns the reranked selection **plus the full pre→post ledger** (`meta.reranker: deterministic_rerank_v1`), and the playground inspector (`RetrievalInspector`/`RagReveal`, verified) renders per-candidate lexical score, rerank score, and rank movement entirely from that backend ledger — nothing fabricated client-side. Still model-free — the "no embeddings, no cross-encoder" boundary holds.
...
6. **Frontend semantics/content batch** — structured cards now *validator-enforced* (no markdown fields), heading hierarchy and contrast fixes, citation popover viewport clamping, operable static citations, focus-ownership rules for cross-surface navigation (`routeCompletion.js` owns destination focus + completion announcements), and the portfolio self-referential link hidden via a content-governed mechanism (verified: `links.json` entries gained `showInUi`/`showInContact` flags, the adapter filters `showInUi: false`, README updated in the same commit). All small, all in the right direction.
...
S1. **Programme jargon shipped in product code:** "PA3-shaped" (`nonCriticalScheduler.js`), `?pa3-retry=` import specifiers, "PA5-shaped" (budget fixtures/tests), "FE-B12" (`validate-content.mjs`), "Branch A / PA5 evidence" (`budgets.json`), "F-15 / FE-A4" and "Performance Branch C" (`routeCompletion.js`). These are task labels from the orchestration workflow; in `docs/performance/` they're the programme record, in shipped source they're opaque to any future reader. One sweep replaces them with plain descriptions.
